...
make astyle-check || exit 1
```
It fixes a bug where the pre-commit could try to read a deleted file and refuse the entire commit despite correct formatting.
...
3. Import `msvc-full-features/AStyleExtension-Cataclysm-DDA.cfg` from your checkout on the `Export/Import` tab using the `Import` button:
