...
[
  {
    ...
    "loading_images": [ "cool.png", "cool2.gif" ],  // Optional. Filenames for any loading screen images the mod may have. Loading screens are only present on the graphical/"tiles" version. Supports .png, and animated .gif where the build's SDL_image is 2.6.0 or newer - on an older one an animation shows its first frame only. Actual loading screen image will be picked randomly from among all mod loading screens, including other loaded mods that have loading screens. Animations advance in real time, but the loading screen only redraws when a loading step finishes, so a long step will drop frames rather than slow the animation down. Note that every frame is held in video memory at full size, so a long animation is expensive: 24 frames at 1362x1020 is over 130MB. GIF delays below 20ms are promoted to 100ms by SDL_image, as they are by web browsers, so author timings at 20ms or more if the exact loop length matters.
